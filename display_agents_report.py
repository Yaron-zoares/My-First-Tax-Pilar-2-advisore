...
try:
    ...
    from yaml_crew_loader import load_crew_from_yaml, get_available_workflows
except ImportError as e:
    print(f"Import error: {e}")
    print("Please ensure all agent modules are available")
    sys.exit(1)

class AgentsReportDisplay:
    """Display comprehensive reports about all agents in the system"""
    ...
    def generate_agents_overview(self) -> Dict[str, Any]:
        ...
        # Main Pillar Two Master Agent
        try:
            pillar_master = PillarTwoMaster()
            overview["agents_by_category"]["main_agent"] = {
                "name": "PillarTwoMaster",
                "role": "OECD Pillar Two Tax Expert",
                "status": "active",
                "tools_count": len(pillar_master._get_tools()),
                "team_members": len(pillar_master.team),
                "capabilities": [
                    "ETR calculations",
        ...
        try:
            ...
            overview["available_workflows"] = workflows
    ...
    def generate_system_status_report(self) -> Dict[str, Any]:
        ...
        try:
            # Test PillarTwoMaster initialization
            pillar_master = PillarTwoMaster()
            status_report["components"]["pillar_two_master"] = {
                "status": "operational",
                "memory_initialized": bool(pillar_master.memory),
                "tools_loaded": len(pillar_master._get_tools()) > 0,
                "team_created": len(pillar_master.team) > 0
            }
        except Exception as e:
            status_report["components"]["pillar_two_master"] = {
                "status": "error",
        ...
        try:
            report_generator = PillarTwoReportGenerator()
            status_report["components"]["report_generator"] = {
                "status": "operational",
                "templates_loaded": bool(report_generator.report_templates),
                "templates_count": len(report_generator.report_templates)
            }
        except Exception as e:
            status_report["components"]["report_generator"] = {
                "status": "error",
        ...
        # Check YAML crew loader
        try:
            workflows = get_available_workflows()
            status_report["components"]["yaml_crew_loader"] = {
                "status": "operational",
    ...
    def _display_overview(self):
        ...
        if main_agent.get("status") == "active":
            print("🎯 MAIN AGENT:")
            ...
            print(f"❌ MAIN AGENT: {main_agent.get('error', 'Unknown error')}")
        ...
        if "members" in team:
            ...
            print(f"❌ TEAM MEMBERS: {team.get('error', 'Unknown error')}")
        
        print()
        
        # YAML Agents
        yaml_agents = overview["agents_by_category"].get("yaml_agents", {})
        if "agents" in yaml_agents:
            print("⚙️ YAML-CONFIGURED AGENTS:")
            for agent in yaml_agents["agents"]:
                print(f"  • {agent['name']} - {agent['role']}")
                print(f"    Tools: {', '.join(agent['tools'])}")
        else:
            print(f"❌ YAML AGENTS: {yaml_agents.get('error', 'Unknown error')}")
